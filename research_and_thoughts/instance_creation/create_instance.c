...
int create_instance(char *size, char* vol_id, bool volume_provided, char *instance_id){//read AWS flag for additional options
  ...
  strcpy(command, "aws ec2 run-instances --image-id ami-9fbbfaf5 --count 1 --output text --key-name ec2-backup-key --subnet-id ");
  //this is an omnios os instance
...
int main(){
  ...
  char instance_id[30];
  create_instance("1", vol_id, true, instance_id);
  printf("vol_id: %s\n", vol_id);
